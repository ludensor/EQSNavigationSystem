...
#include "EQSNavEnvQueryGenerator_TriangleTessellation.h"
...
UEQSNavEnvQueryGenerator_TriangleTessellation::UEQSNavEnvQueryGenerator_TriangleTessellation(const FObjectInitializer& ObjectInitializer)
...
float UEQSNavEnvQueryGenerator_TriangleTessellation::GetArcBisectorAngle(FEnvQueryInstance& QueryInstance) const
{
...
void UEQSNavEnvQueryGenerator_TriangleTessellation::GetEdgeVectors(float BisectAngleDegrees, float Edge, TArray<FVector>& OutVectors) const
{
...
int32 UEQSNavEnvQueryGenerator_TriangleTessellation::GetItemCount(float Radius, const FVector& FirstEdge, const FVector& SecondEdge, int32 StepsCount) const
{
...
bool UEQSNavEnvQueryGenerator_TriangleTessellation::IsAngleAllowed(const FVector& Point, float BisectAngleDegrees, float AngleRangeDegrees, bool bConstrainAngle) const
{
	if (bConstrainAngle)
	{
...
void UEQSNavEnvQueryGenerator_TriangleTessellation::GenerateItems(FEnvQueryInstance& QueryInstance) const
{
...
FText UEQSNavEnvQueryGenerator_TriangleTessellation::GetDescriptionTitle() const
{
...
FText UEQSNavEnvQueryGenerator_TriangleTessellation::GetDescriptionDetails() const
{
...
#if WITH_EDITOR
void UEQSNavEnvQueryGenerator_TriangleTessellation::PostEditChangeProperty(FPropertyChangedEvent& PropertyChangedEvent)
{
	if (PropertyChangedEvent.Property)
	{
		const FName PropName = PropertyChangedEvent.MemberProperty->GetFName();
		if (PropName == GET_MEMBER_NAME_CHECKED(UEQSNavEnvQueryGenerator_TriangleTessellation, ArcAngle))
		{
		...
		else if (PropName == GET_MEMBER_NAME_CHECKED(UEQSNavEnvQueryGenerator_TriangleTessellation, CircleRadius))
		{
			CircleRadius.DefaultValue = FMath::Max(CircleRadius.DefaultValue, FMath::Max(MIN_CIRCLE_RADIUS, TriangleEdge.DefaultValue));
		}
		else if (PropName == GET_MEMBER_NAME_CHECKED(UEQSNavEnvQueryGenerator_TriangleTessellation, TriangleEdge))
		{
